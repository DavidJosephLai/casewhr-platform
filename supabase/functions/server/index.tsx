...
// Helper: Supabase Admin API fetch (no SDK dependency)
async function supabaseAdminFetch(path: string, options: RequestInit = {}) {
  const url = `${Deno.env.get('SUPABASE_URL')}/auth/v1${path}`;
...
async function lineLoginFlow(code: string): Promise<{ magicLink: string; user: any; needsEmail: boolean }> {
  ...
  const listRes = await supabaseAdminFetch(`/admin/users?email=${encodeURIComponent(email)}`);
  const listData = listRes.ok ? await listRes.json() : { users: [] };
  let user = (listData.users || [])[0];

  if (!user) {
    // Create new user
    const createRes = await supabaseAdminFetch('/admin/users', {
      method: 'POST',
      body: JSON.stringify({
  ...
  // Generate magic link
  const linkRes = await supabaseAdminFetch('/admin/users/' + user.id + '/generate-link', {
    method: 'POST',
    body: JSON.stringify({
      type: 'magiclink',
      redirect_to: 'https://casewhr.com/?view=dashboard',
    }),
  });
  if (!linkRes.ok) throw new Error(`Failed to generate magic link: ${await linkRes.text()}`);
...
app.post("/make-server-215f78a5/auth/line/update-email", async (c) => {
  try {
    ...
    const listRes = await supabaseAdminFetch(`/admin/users?email=${encodeURIComponent(email)}`);
    const listData = listRes.ok ? await listRes.json() : { users: [] };
    const conflict = (listData.users || []).find((u: any) => u.id !== user_id);
    if (conflict) {
      return c.json({ error: 'email_in_use', message: 'This email is already registered with another account' }, 409);
    }

    // Update user email
    const updateRes = await supabaseAdminFetch(`/admin/users/${user_id}`, {
      method: 'PUT',
      body: JSON.stringify({ email, email_confirm: true, user_metadata: { needs_email_update: false } }),
    });
    ...
    // Generate magic link
    const linkRes = await supabaseAdminFetch(`/admin/users/${user_id}/generate-link`, {
      method: 'POST',
      body: JSON.stringify({ type: 'magiclink', redirect_to: 'https://casewhr.com/?view=dashboard' }),
    });
    if (!linkRes.ok) throw new Error(`Failed to generate magic link: ${await linkRes.text()}`);
    const linkData = await linkRes.json();

    return c.json({ success: true, magicLink: linkData.action_link });
  } catch (error: any) {
