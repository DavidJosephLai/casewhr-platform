...
app.get("/make-server-215f78a5/ai-seo/reports", async (c) => {
  try {
    console.log('📋 [AI SEO] Loading all SEO reports...');

    // 從 KV Store 獲取所有 ai_seo_page: 開頭的記錄
    const allReports = await kv.getByPrefix('ai_seo_page:') || [];
    
    console.log(`✅ [AI SEO] Found ${allReports.length} SEO reports`);
    ...
    const reports = allReports.map((report: any) => {
      ...
      return {
        key: `ai_seo_page:${value.url}`,
        value: {
          url: value.url || '',
          title: value.title || '',
          description: value.description || '',
          keywords: (value.keywords || '').split(',').map((k: string) => k.trim()).filter(Boolean),
          content: JSON.stringify(value, null, 2), // 完整內容
          generatedAt: value.generatedAt || value.updatedAt || new Date().toISOString(),
          status: 'published', // 默認狀態
        },
      };
...
// 刪除 AI SEO 報告
app.delete("/make-server-215f78a5/ai-seo/reports/:key", async (c) => {
  try {
    const key = decodeURIComponent(c.req.param('key'));
    console.log('🗑️ [AI SEO] Deleting report:', key);

    await kv.del(key);
    
    console.log('✅ [AI SEO] Report deleted successfully');
